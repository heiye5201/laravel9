...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('integral_goods', function (Blueprint $table) {
            ...
            $table->text('goods_images')->nullable()->comment('商品图片');
            $table->string('goods_master_image', 150)->default('')->comment('主图');
            $table->unsignedInteger('goods_sale')->default(0)->nullable()->comment('销售量');
            $table->unsignedTinyInteger('goods_status')->default(0)->comment('上下架');
            $table->unsignedTinyInteger('is_recommend')->default(0)->comment('推荐');
            $table->text('goods_content')->nullable()->comment('详情');
            $table->text('goods_content_mobile')->nullable()->comment('手机端详情');
            $table->timestamps();
            $table->softDeletes();
        });
    }
