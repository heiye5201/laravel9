...
class DashboardController extends Controller
{
    ...
    public function all(Request $request)
    {
        ...
        $data['week_price'] = Order::query()->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfWeek()->startOfDay()->toDateTimeString())->sum('total_price'); // 这周销售额
        $data['last_week_price'] = Order::query()->where('order_status', '>', 1)->where('pay_time', '<', Carbon::now()->startOfWeek()->startOfDay()->toDateTimeString())->where('pay_time', '>=', Carbon::now()->subWeeks(1)->startOfWeek()->startOfDay()->toDateTimeString())->sum('total_price'); // 上周销售额
        $data['month_price'] = Order::query()->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfMonth()->startOfDay()->toDateTimeString())->sum('total_price'); // 这月销售额
        $data['last_month_price'] = Order::query()->where('order_status', '>', 1)->where('pay_time', '<', Carbon::now()->startOfMonth()->startOfDay()->toDateTimeString())->where('pay_time', '>=', Carbon::now()->subMonths(1)->startOfMonth()->startOfDay()->toDateTimeString())->sum('total_price'); // 上月销售额
        $data['order_wait'] = Order::query()->where('order_status', 1)->count(); // 等待支付
        $data['order_send'] = Order::query()->where('order_status', 2)->count(); // 等待发货
        $data['order_complete'] = Order::query()->where('order_status', 6)->count(); // 完成
        $data['order_refund'] = Order::query()->where('order_status', 5)->count(); // 售后
        ...
        // 获取店铺销售排行
        $data['list'] = Store::query()->select('store_name', 'id')->withCount(['orders'=>function ($q) {
    ...
    public function order(Request $request)
    {
        ...
        $data['plot'] = DB::select($sql, [$first_time,$end_time]);
        // 获取店铺销售排行
        $data['store'] = Store::query()->select('store_name', 'id')->withCount(['orders'=>function ($q) {
