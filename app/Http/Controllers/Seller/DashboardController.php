...
class DashboardController extends Controller
{
    ...
    public function all()
    {
        ...
        $data['total_price'] = Order::query()->where('store_id', $store_id)->where('order_status', '>', 1)->sum('total_price');// 总销售额
        $data['today_price'] = Order::query()->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', now()->startOfDay()->toDateTimeString())->sum('total_price'); // 今日销售额
        $data['yesterday_price'] = Order::query()->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', now()->startOfDay()->toDateTimeString())->where('pay_time', '>=', Carbon::yesterday()->toDateTimeString())->sum('total_price'); // 昨日销售额
        $data['week_price'] = Order::query()->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfWeek()->startOfDay()->toDateTimeString())->sum('total_price'); // 这周销售额
        $data['last_week_price'] = Order::query()->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', Carbon::now()->startOfWeek()->startOfDay()->toDateTimeString())->where('pay_time', '>=', Carbon::now()->subWeeks(1)->startOfWeek()->startOfDay()->toDateTimeString())->sum('total_price'); // 上周销售额
        $data['month_price'] = Order::query()->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfMonth()->startOfDay()->toDateTimeString())->sum('total_price'); // 这月销售额
        $data['last_month_price'] = Order::query()->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', Carbon::now()->startOfMonth()->startOfDay()->toDateTimeString())->where('pay_time', '>=', Carbon::now()->subMonths(1)->startOfMonth()->startOfDay()->toDateTimeString())->sum('total_price'); // 上月销售额
        $data['order_wait'] = Order::query()->where('store_id', $store_id)->where('order_status', 1)->count(); // 等待支付
        $data['order_send'] = Order::query()->where('store_id', $store_id)->where('order_status', 2)->count(); // 等待发货
        $data['order_complete'] = Order::query()->where('store_id', $store_id)->where('order_status', 6)->count(); // 完成
        $data['order_refund'] = Order::query()->where('store_id', $store_id)->where('order_status', 5)->count(); // 售后
        ...
        // 获取商品销售排行
        $data['list'] = Goods::query()->select('goods_name', 'id')->withCount(['order_goods as orders_count'=>function ($q) {
    ...
    public function order()
    {
        ...
        // 获取店铺销售排行
        $data['store'] = Goods::query()->select('goods_name', 'id')->withCount(['order_goods as orders_count'=>function ($q) {
            $q->select(DB::raw('sum(buy_num)'));
        }])->orderBy('orders_count', 'desc')->take(10)->get();
        // 获取商品销售排行
        $data['goods'] = Goods::query()->select('goods_name', 'id')->withCount(['order_goods as orders_count'=>function ($q) {
