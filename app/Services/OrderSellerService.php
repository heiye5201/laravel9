...
class OrderSellerService extends BaseService
{
    // 要删除的购物车数据
    protected $cartId = [];
    ...
    public function createOrder($reqData)
    {
        ...
        $seckillService = app(SeckillService::class);
        $fullReductionService = app(FullReductionService::class);
        $collectiveService = app(CollectiveService::class);
        // 循环生成订单 多个商家则生成多个订单
        try {
            DB::beginTransaction();
            $resp_data = [];
            foreach ($rs['data'] as $k => $v) {
                ...
                if ($order_data['coupon_id'] > 0) {
                    $cpli = CouponLog::query()->where('id', $order_data['coupon_id'])
                ...
                if ($collective_id != 0) {
                    $collective_resp = Collective::query()->where('goods_id', $v['goods_list'][0]['id'])->first();
                    if ($collective_resp) {
    ...
    private function delCart()
    {
        if (!empty($this->cartId)) {
            try {
                Cart::query()->whereIn('id', $this->cartId)->delete();
            } catch (\Exception $e) {
    ...
    public function createOrderAfter()
    {
        ...
        $params = $check['data'];
        $list = Order::query()->whereIn('id', $params['order_id'])->with('order_goods', 'refund')->get();
    ...
    public function orderStock($goods_id, $sku_id, $num, $is_type = 0)
    {
        try {
            DB::beginTransaction();
            if (empty($sku_id)) {
                $goods_model = Goods::query()->lockForUpdate()->find($goods_id);
                if (!$goods_model) {
                ...
                $goods_sku_model = GoodsSku::query()->lockForUpdate()->find($sku_id);
                if (!$goods_sku_model) {
                    throw new \Exception(__('tip.order.handleErr') . ' - stock lock');
                }
                if (empty($is_type)) {
    ...
    {
        try {
            $goods_model = Goods::query()->find($goods_id);
            if (!$goods_model) {
    ...
    protected function createPayOrder($opt = [])
    {
        ...
        try {
            ...
            $op = OrderPay::query()->where('belong_id', $params['user_id'])->orderBy('id', 'desc')->first();
            if ($op) {
    ...
    public function editOrderStatus($order_id, $order_status, $auth = "users")
    {
        try {
            DB::beginTransaction();
            $order_model = Order::query()->lockForUpdate()->find($order_id);
            if (!$order_model) {
            ...
            // 商家可操作
            if ($auth == 'seller') {
                switch ($order_model->order_status) {
                    case 0:
                        //取消
                        throw new \Exception(__('tip.this_order_cancel'));
                        break;
                    case 1:
                        //待支付
                        throw new \Exception(__('tip.this_order_wait_pay'));
                        break;
                    case 2:
                        //待发货
                        if ($order_model->apply_status==0||$order_model->apply_status==1) {
                            throw new \Exception(__('tip.this_order_wait_pay'));
                        }
                        break;
                }
            ...
            switch ($order_status) {
                case 0: // 取消订单
                    ...
                    $og_list = OrderGoods::query()->select('goods_id', 'sku_id', 'buy_num')->where('order_id', $order_id)->get();
                    // 库存修改
                    foreach ($og_list as $v) {
                        $this->orderStock($v['goods_id'], $v['sku_id'], $v['buy_num'], 1);
                    }
                    // 如果有优惠券则修改优惠券
                    CouponLog::query()->where('order_id', $order_id)->update(['status' => 0, 'order_id' => 0]);
                    break;
                case 1: // 等待支付
    ...
    public function checkAddress()
    {
        ...
        $address_info = Address::query()->find($id);
    ...
    public function getApplyStatus($order_info){
        $cn = '待审核';
        if ($order_info['order_status']>2) {
            $cn = __('tip.applyStatusSuccess');
        } else {
            switch ($order_info['apply_status']) {
                case 0:
                    $cn = __('tip.waitStatus');
                    break;
                case 1:
                    $cn = __('tip.applyStatusSuccess');
                    break;
                case 2:
                    $cn = __('tip.applyStatusCancel');
                    break;
            }
        }
        return $cn;
    ...
    {
        $cn = '未知订单';
        if (($order_info['apply_status']==0||$order_info['apply_status']==2)
    ...
    public function applyStatus()
    {
        ...
        $params = $check['data'];
        $canApplyCount = Order::query()
            ->whereIn('id', $params['order_id'])->where('apply_status',0)->count();
        if (count($params['order_id'])>$canApplyCount) {
            return $this->formatError(__('tip.orderCanApplyStatus'));
        }
        $count = Order::query()
            ->whereIn('id', $params['order_id'])->update(['apply_status'=>$params['apply_status']]);
