...
class FavoriteService extends BaseService
{
    ...
    public function add()
    {
        ...
        $userId = $this->getUserId('users');
        $fav_info = Favorite::query()->where(['user_id' => $userId, 'out_id' => $out_id, 'is_type' => request()->is_type])->first();
        if (!empty($fav_info)) {
            Favorite::query()->where(['user_id' => $userId, 'out_id' => $out_id, 'is_type' => request()->is_type])->delete();
    ...
    public function isFav()
    {
        ...
        $fav_info = Favorite::query()->where(['user_id' => $userId, 'out_id' => $out_id, 'is_type' => request()->is_type])->exists();
        if (empty($fav_info)) {
