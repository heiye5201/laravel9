...
class PaymentService extends BaseService
{
    ...
    public function payment($paymentName = 'wechat', $device = 'web', $config = 'default')
    {
        ...
        try {
            ...
            $orderPay = OrderPay::query()->where('pay_no', $result->out_trade_no)->first();
            ...
            if ($orderPay->is_recharge == 0) {
                ...
                // 分销处理
                try {
                    app(DistributionService::class)->addDisLog($oid_arr);
                } catch (\Exception $e) {
    ...
    public function pay($paymentName = 'wechat', $device = 'web', $orderPay = [], $recharge = false, $config = 'default')
    {
        ...
        if ($paymentName == 'balance') {
            ...
            $orderPayModel = OrderPay::query()->where('pay_no', $orderPay['pay_no'])->first();
            ...
            $orderGoodsList = OrderGoods::query()->whereIn('order_id', $orderIds)->get();
    ...
    public function check($orderId)
    {
        ...
        $order = Order::query()->where('id', $orderId)->where('order_status', '>', 1)->first();
        if (!$order) {
