...
class SmsService extends BaseService
{
    ...
    public function sendSms($phone, $name)
    {
        ...
        $configService = app(ConfigsService::class);
        $alisms = $configService->getFormatConfig('sms'); // 获取下签名密钥
        $sms = Sms::query()->where('name', $name)->first();// 获取签名配置
        if (!$sms) {
    ...
    public function checkSms($phone, $code)
    {
        ...
        if (empty($smsInfo = SmsLog::query()->where([
            'ip' => request()->getClientIp(),
            'content' => $code,
            'status' => 1,
            'phone' => $phone,
    ...
    {
        // IP 加时间验证
        $smsInfo = SmsLog::query()->where('ip', request()->getClientIp())->where('phone', $phone)->where('name', $name)->orderBy('id', 'desc')->first();
        // 验证码失效
        if ($smsInfo) {
            $ct = strtotime($smsInfo->created_at->format('Y-m-d H:i:s'));
            if (($ct + 30) > time()) {
