                ...
                <el-form v-if="dialogParams.view && dialogParams.view.column.length>0"  label-position="right"  :label-width="dialogParams.labelWidth" :fullscreen="dialogParams.fullscreen">
                    <el-row :gutter="20">
                        <el-col v-for="(v,k) in dialogParams.view.column" :key="k" :span="v.span || dialogParams.span">
                            ...
                            <div v-if="v.type!='array'" class="table-form-content">
                                <el-form-item :label="v.label+' : '" :prop="v.value">
                    ...
                    <div v-for="(v,k) in dialogParams.view.column" :key="k" :span="v.span || dialogParams.span">
                        <div class="data_list" v-if="v.type=='array'">
                            <el-row v-for="(item,key) in formData.view[v.value]">
                                <el-col >
                                    {{item.goods_name}} - {{item.sku_name}}
                                </el-col>
                               <el-col><img :src="item.goods_image" /></el-col>
                                <el-col>x {{item.buy_num}}</el-col>
                                <el-col style="color:red">{{item.goods_price}}</el-col>
                            </el-row>
                        </div>
                    </div>
                </el-form>
