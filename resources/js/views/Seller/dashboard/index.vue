<template>
    <base-view :hideMian="true">
        <template  class="dashboard" #main_view_sub>
            <div class="top_div_block">
                <div class="top_div_item">
                    <div class="admin_card" >
                        <div class="title">{{$t('seller.dashboard.total_price')}}</div>
                        <div class="content">
                            <div class="total_price">{{$t('btn.money')}} {{data.info.total_price||'0.00'}}</div>
                            <div class="total_rate">
                                <span>{{$t('seller.dashboard.week_rate')}} {{data.info.week_rate||'0.00'}} %<el-icon v-if="data.info.week_rate>=0" style="margin-left:5px;color:red;" ><CaretTop /></el-icon><el-icon v-else style="margin-left:5px;color:green;"  ><CaretBottom /></el-icon></span>
                                <span>{{$t('seller.dashboard.day_rate')}} {{data.info.day_rate||'0.00'}} %<el-icon v-if="data.info.day_rate>=0" style="margin-left:5px;color:red;" ><CaretTop /></el-icon><el-icon v-else style="margin-left:5px;color:green;"  ><CaretBottom /></el-icon></span>
                            </div>
                            ...
                            <div class="today_price">{{$t('seller.dashboard.today_price')}}：{{$t('btn.money')}} {{data.info.today_price||'0.00'}}</div>
                        </div>
                    </div>
                </div>
                <div class="top_div_item">
                    <div class="admin_card" >
                        <div class="title">{{$t('seller.dashboard.order_info')}}</div>
                        <div class="content">
                            <el-row :gutter="12">
                                <el-col :span="12" :xs="{ span: 24 }" :lg="{ span: 12 }">
                                    <div class="color_block">
                                        <div><el-tag>{{$t('seller.dashboard.order_wait')}}</el-tag></div>
                                        <div class="color_blcok_font">{{data.info.order_wait||'0'}}</div>
                                    </div>
                                </el-col>
                                <el-col :span="12" :xs="{ span: 24 }" :lg="{ span: 12 }">
                                    <div class="color_block">
                                        <div><el-tag type="success">{{$t('seller.dashboard.order_complete')}}</el-tag></div>
                            ...
                            <el-row :gutter="12">
                                <el-col :span="12" :xs="{ span: 24 }" :lg="{ span: 12 }">
                                    <div class="color_block">
                                        <div><el-tag type="danger">{{$t('seller.dashboard.order_send')}}</el-tag></div>
                                        <div class="color_blcok_font">{{data.info.order_send||'0'}}</div>
                                    </div>
                                </el-col>
                                <el-col :span="12" :xs="{ span: 24 }" :lg="{ span: 12 }">
                                    <div class="color_block">
                                        <div><el-tag type="warning">{{$t('seller.dashboard.order_refund')}}</el-tag></div>
            ...
            <div class="admin_card" >
                <div class="title">
                    <div class="right_block">
                        <ul>
                            <li :class="params.is_type==0?'ck':''" @click="typeChange(0)">{{$t('seller.dashboard.this_week')}}</li>
                            <li :class="params.is_type==1?'ck':''" @click="typeChange(1)">{{$t('seller.dashboard.this_year')}}</li>
                        </ul>
                        <div class="daterange"><el-date-picker type="daterange" v-model="params.created_at" @change="onChange" format="YYYY-MM-DD" /></div>
                    </div>
                    {{$t('seller.dashboard.sales_trend')}}
                </div>
                <div class="content">
                    <el-row :gutter="22">
                        <el-col :span="16" >
                        ...
                        <el-col :span="8">
                            <div class="sort_list">
                                <div class="list_title">{{$t('seller.dashboard.sales_ranking')}}</div>
            ...
            <div class="admin_card" >
                <div class="title">{{$t('seller.dashboard.order_quantity_trend')}}</div>
                <div class="content">
    ...
    setup(props) {
        ...
        onMounted(() => {
            ...
            data.column = new Column('container'+data.random,{
                ...
                meta: {
                    time: {
                        alias: proxy.$t('common.time'),
                    },
                    num: {
                        alias: proxy.$t('common.sales_price'),
                    },
                },
                columnBackground: {
                    style: {
            ...
            data.line = new Line('user_plot'+data.random,{
                ...
                meta: {
                    ...
                    num: {
                        alias: proxy.$t('common.order_quantity'),
                    },
                },
            })
            data.line.render()
