            ...
            <div class="goods_info_top_right" >
                ...
                <div class="goods_info_group">
                    <template v-if="data.seckills">
                        <div class="goods_info_price"><span>{{$t('home.goods.price_spike')}}：</span>{{$t('btn.money')}}{{data.goods_info.goods_price*(1-data.seckills.discount/100)||'0.00'}}</div>
                        <div class="goods_info_market_price"><span>{{$t('home.goods.market_value')}}：</span><div class="overx_goods_info">{{$t('btn.money')}}{{data.goods_info.goods_market_price||'0.00'}}</div></div>
                    </template>
                    <template v-else>
                        <div class="goods_info_price"><span>{{$t('home.goods.store_price')}}：</span>{{$t('btn.money')}}{{data.goods_info.goods_price||'0.00'}}</div>
                        <div class="goods_info_market_price"><span>{{$t('home.goods.market_value')}}：</span><div class="overx_goods_info">{{$t('btn.money')}}{{data.goods_info.goods_market_price||'0.00'}}</div></div>
                    </template>
                    ...
                    <div class="goods_info_sale_num">{{$t('home.goods.total_sales')}}<em color="#ca151e">{{data.goods_info.goods_sale||0}}</em></div>
                    <div class="goods_info_phone_read">{{$t('home.goods.phone_view')}}<i style="font-size:16px" class="fa fa-qrcode"></i></div>
                    ...
                    <div class="coupons_block" v-if="data.coupons.length>0">
                        <ul>
                            <li v-for="(v,k) in data.coupons" :key="k" @click="receiveCoupon(v.id)">
                                ...
                                <div class="block">
                                    <div>{{$t('home.goods.coupon')}}</div>
                                    <div>{{$t('home.goods.full')}}{{$t('btn.money')}}{{v.use_money}}{{$t('home.goods.can_use')}}</div>
                                </div>
                            </li>
                        </ul>
                    </div>
                ...
                <div class="goods_info_active">
                    <div class="goods_skill" v-if="data.seckills">
                        <span><el-icon style="margin:0 10px"><Timer /></el-icon>{{$t('home.goods.join_spike_activity')}}</span>
                        <span class="span_time">{{$t('home.goods.distance_end')}} {{data.seckills.format_time||'0 天 00 时 00 分 00 秒'}}</span>
                    </div>
                    <div class="goods_skill tuan_active" v-if="data.collectives"  @click="data.collective_id=-1;buy()">
                        <span><el-icon style="margin:0 10px"><UserFilled /></el-icon>{{$t('home.goods.join_team_activity')}} ( {{$t('home.goods.click_team')}} ) </span>
                        <span class="span_time">{{$t('home.goods.group_buying_price')}}：{{$t('btn.money')}} {{R.formatFloat(data.goods_info.goods_price*(1-data.collectives.discount/100)||'0.00')}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {{$t('home.goods.need')}} {{data.collectives.need}} {{$t('home.collectives.people')}}</span>
                    </div>
                        ...
                        <el-carousel autoplay  :direction="'vertical'" height="30px" indicator-position="none">
                            <el-carousel-item  v-for="(v,k) in data.collective_list" :key="k">
                                <div class="tuan_item">
                                    ...
                                    <div class="btn" @click="data.collective_id=v.id;buy()">{{$t('home.goods.join_team')}}</div>
                                    <div class="orders_count">{{$t('home.goods.has_join_team')}} {{v.orders_count}} / {{v.need}} {{$t('home.collectives.people')}}</div>
                                </div>
                            </el-carousel-item>
                        </el-carousel>
                    </div>
                ...
                <div class="goods_info_full_reduction" v-if="data.full_reductions.length>0">
                    <span class="title">{{$t('home.goods.activity')}}：</span>
                    <span class="act" v-for="(v,k) in data.full_reductions" :key="k"> {{$t('home.goods.full')}}{{$t('btn.money')}}{{v.use_money}}{{$t('home.goods.reduce')}}{{$t('btn.money')}}{{v.money}}；</span>
                </div>
                <!-- 满减 E -->
                ...
                <div class="goods_info_num">
                    <div class="goods_info_num_title">{{$t('home.goods.buy_count')}}：</div>
                    ...
                    <div class="goods_info_num_stock">&nbsp;&nbsp;{{data.goods_info.goods_stock}} {{$t('home.goods.stock')}}</div>
                    <div class="clear"></div>
                </div>
                <div class="goods_info_btn">
                    ...
                    <template v-if="data.seckills">
                        <div class="goods_info_buy" @click="buy()"><img :src="require('@/assets/Home/buy.png').default" /> {{$t('home.goods.buy_rob_now')}}</div>
                    </template>
                    <template v-else>
                        <div class="goods_info_buy" @click="buy()"><img :src="require('@/assets/Home/buy.png').default" /> {{$t('home.goods.buy_now')}}</div>
                        <div class="goods_info_add_cart" @click="add_cart()"><img :src="require('@/assets/Home/cart.png').default" />{{$t('home.goods.add_cart')}}</div>
                    </template>
            ...
            <div class="left_item">
                <div class="store_info">
                    <div class="store_title">
                        <span class="tip">{{$t('home.goods.shop')}}</span>
                        <span class="title">{{data.store_info['store_name']||$t('home.loading')}}</span>
                    </div>
                    <div class="rate">
                        <span style="padding-top:2px;margin-right:10px">{{$t('home.goods.synthetical_score')}}</span>
                    ...
                    <div class="store_rate">
                        <div class="title">{{$t('home.goods.shop_score')}}：</div>
                        <div class="item">
                            <span style="float:left;padding-top:2px;margin-right:10px;line-height:32px">{{$t('home.goods.desc_score')}}</span>
                        ...
                        <div class="item">
                            <span style="float:left;padding-top:2px;margin-right:10px;line-height:32px">{{$t('home.goods.service_score')}}</span>
                        ...
                        <div class="item">
                            <span style="float:left;padding-top:2px;margin-right:10px;line-height:32px">{{$t('home.goods.delivery_speed')}}</span>
                    ...
                    <div class="store_com" style="margin-top:10px">{{$t('home.goods.shop_name')}}：<em style="color:#999">{{data.store_info.store_company_name}}</em></div>
                    <div class="store_com" style="margin-bottom:10px">{{$t('home.goods.shop_address')}}：<em style="color:#999">{{data.store_info.area_info+' '+data.store_info.store_address}}</em></div>
                    <div class="btn">
                        <span class="navstore" @click="$router.push('/store/'+data.store_info.id)">{{$t('home.carts.store')}}</span>
                        <span class="contact" @click="$refs['chat'].openChat()">{{$t('home.goods.contact_customer')}}</span>
                        <div class="clear"></div>
                    </div>
                </div>
                <!-- // 销售排行 -->
                <div class="store_info">
                    <div class="store_title"><span class="title">{{$t('home.goods.sale_rank')}}</span></div>
                    ...
                    <div style="line-height:90px;text-align:center;color:#999;" v-else>{{$t('home.goods.not_goods')}}</div>
                </div>
            </div>
            <div class="right_item">
                 <el-tabs model-value="1" @tab-click="tabClick">
                    <el-tab-pane name="1" :label="$t('home.goods.detail')" >
                    ...
                    <el-tab-pane name="2" :label="'用户评价 ('+(data.comment_statistics.all||0)+')'" >
                        ...
                        <div class="comment_list_top">
                            <div class="left_bfb">
                                ...
                                <span>{{$t('home.goods.favorable_rate')}}</span>
                            </div>
                            <div class="right_comment_list">
                                <ul>
                                    <li :class="data.params.is_type==0?'red':''" @click="comment_type_click(0)">{{$t('home.carts.all')}} ({{data.comment_statistics.all||0}})</li>
                                    <li :class="data.params.is_type==1?'red':''" @click="comment_type_click(1)">{{$t('home.goods.praise')}} ({{data.comment_statistics.good||0}})</li>
                                    <li :class="data.params.is_type==2?'red':''" @click="comment_type_click(2)">{{$t('home.goods.medium')}} ({{data.comment_statistics.commonly||0}})</li>
                                    <li :class="data.params.is_type==3?'red':''" @click="comment_type_click(3)">{{$t('home.goods.negative')}} ({{data.comment_statistics.bad||0}})</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="comment_list_block">
                            <ul>
                                <li v-for="(v,k) in data.comments" :key="k">
                                    ...
                                    <div class="comment_star"><em>{{$t('home.goods.evaluation_score')}}：</em><div class="store_star_in"><el-rate disabled v-model="v.score" :score-template="'{value} '+$t('home.store.score')" text-color="#F7BA2A" show-score></el-rate></div></div>
                                    <div class="comment_content">{{$t('home.goods.evaluation_content')}}：<em style="color:#999">{{v.content||'good!~'}}</em></div>
                                    ...
                                    <div class="reply" v-if="v.reply!=''">
                                        ...
                                        <div class="comment_nickname">{{$t('home.goods.after_sales_service')}}</div>
                                        <div class="comment_content">{{$t('home.goods.reply_content')}}：<em >{{v.reply||'good!~'}}</em></div>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    ...
                    <el-tab-pane name="3" :label="$t('config.configStore.storeService')" force-render>
