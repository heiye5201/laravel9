<template>
    <div class="create_order_1 w1200">
        ...
        <!-- 地址信息选择 S -->
        <div class="block">
            <div class="title">{{$t('home.integral.send_address')}}</div>
            ...
            <div class="empty_address" v-else>
                {{$t('home.not_set_address')}}，{{$t('home.go_setting')}}<router-link to="/user/address">{{$t('btn.setting')}}</router-link>
            </div>
        </div>
        <!-- 地址信息选择 E -->
        ...
        <!-- 预生成订单信息 S -->
        <div class="block">
            <div class="title">{{$t('home.carts.goods_detail1')}}</div>
                ...
                <div class="store_list" v-for="(v,k) in data.order" :key="k">
                    <div class="store_title">
                        <div>
                            ...
                            <div v-if="v.is_coupon" class="float_right coupon_order">
                                <el-icon color="#ca151e" size="18"><PriceTag /></el-icon>{{$t('home.carts.coupon_code')}}：
                                <el-select style="width:130px;margin-right:10px;" v-model="v.coupon_id">
                                    <el-option :value="0" :label="$t('home.carts.not_use_coupon')"></el-option>
                                    <el-option v-for="(val,key) in v.coupons" :key="key" :label="$t('btn.money')+val.money+$t('home.carts.coupon_code')" :value="val.id" >
                                    </el-option>
                                </el-select>
                            </div>
                            <div class="clear"></div>
            ...
            <div class="remark">
                <div class="goods_th">{{$t('home.carts.content')}}</div>
            ...
            <div class="sum_block">
                <div class="total">{{$t('home.carts.total_amount')}}：<span>￥{{data.total}}</span>( {{$t('home.carts.exit_freight_or_discount')}} )</div>
                <div :class="loading?'btn hide':'btn'" @click="createOrder">{{data.loading?$t('home.loading'):$t('home.carts.create_order')}}</div>
                <div class="clear"></div>
            </div>
        </div>
        <!-- 预生成订单信息 E -->
