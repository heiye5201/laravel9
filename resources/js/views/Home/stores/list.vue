    ...
    <div class="store_list w1200">
        <div class="breadcrumb">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>{{$t('home.store_index.shop_street')}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
                ...
                <li v-for="(v,k) in data.list" :key="k">
                    <div class="left_block" @click="$router.push('/store/'+v.id)">
                        ...
                        <div class="right_item">
                            ...
                            <div class="qy"><span class="stitle">{{$t('home.company')}}：</span>{{v.store_company_name||''}}</div>
                            <div class="address" title="12"><span class="stitle">{{$t('config.configStore.storeArea')}}：</span>{{(v.area_info||'')+' '+(v.store_address||'')}}</div>
                        </div>
                        <div class="comment_rate"></div>
                    </div>
                    <div class="center_block">
                        {{$t('home.goods.favorable_rate')}}：100%
                    </div>
                    <div class="right_block">
                        <div class="jl">{{$t('home.collectives.distance')}}：<span style="color:#ca151e;font-size:16px">{{v.distance}}</span></div>
                        <div class="btn" @click="$router.push('/store/'+v.id)">{{$t('home.carts.store')}}</div>
                    </div>
                </li>
            </ul>
