<template>
    <div class="user_order">
              ...
              <div class="order_list" v-if="data.list.length>0">
                <div class="order_item" v-for="(v,k) in data.list" :key="k">
                    ...
                    <div class="order_item_btn" >
                        <div class="default_btn orderbtn"  @click="get_order_info(v.id)">{{$t('user_center.integrals.view_logistics')}}</div>
                    </div>
                </div>
        ...
        <!-- 物流 -->
        <a-modal v-model="visible" :title="$t('user_center.integrals.logistics')" :footer="null">
...
export default {
    ...
    setup(props) {
        ...
        const data = reactive({
            ...
            list:[],
            searchConfig:[
                {label:proxy.$t('user_center.integrals.order_sn'),name:'order_no',type:'text'},
            ],
            order_info:{
                delivery_list:[],
            },
