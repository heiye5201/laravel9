    ...
    <div class="user_default2">
        <div class="user_info_balance">
            <dl class="item1">
                ...
                <dd class="title">{{$t('default.user_money')}}</dd>
                <dd class="money">{{data.user_info.money||'0.00'}} {{$t('home.carts.price_unit')}}</dd>
            </dl>
            <dl class="item2">
                <dt><i class="fa fa-btc" /></dt>
                <dd class="title">{{$t('default.user_integral')}}</dd>
                <dd class="money">{{data.user_info.integral||'0.00'}} {{$t('home.integral.point')}}</dd>
            </dl>
            <dl class="item3">
                <dt><i class="fa fa-credit-card" /></dt>
                <dd class="title">{{$t('default.frozen_money')}}</dd>
                <dd class="money">{{data.user_info.frozen_money||'0.00'}} {{$t('home.carts.price_unit')}}</dd>
            </dl>
            <div class="clear"></div>
        </div>
        ...
        <div class="order_status_block">
            <ul>
                <li>
                    ...
                    <span><el-badge :value="data.count[0]" :hidden="data.count[0]==0" :max="99">{{$t('default.wait_pay')}}</el-badge></span>
                    </router-link>
                </li>
                <li>
                    <router-link :to="{name:'user_order',params:{status:2}}">
                    <i class="fa fa-clock-o" />
                    <span><el-badge :value="data.count[1]" :hidden="data.count[1]==0" :max="99">{{$t('default.wait_send')}}</el-badge></span>
                    </router-link>
                </li>
                <li>
                    <router-link :to="{name:'user_order',params:{status:3}}">
                    <i class="fa fa-briefcase" />
                    <span><el-badge :value="data.count[2]" :hidden="data.count[2]==0" :max="99">{{$t('default.wait_received')}}</el-badge></span>
                    </router-link>
                </li>
                <li>
                    <router-link :to="{name:'user_order',params:{status:4}}">
                    <i class="fa fa-commenting-o" />
                    <span><el-badge :value="data.count[3]" :hidden="data.count[3]==0" :max="99">{{$t('default.wait_common')}}</el-badge></span>
                    </router-link>
                </li>
                <li>
                    <router-link :to="{name:'user_order',params:{status:5}}">
                    <i class="fa fa-history" />
                    <span><el-badge :value="data.count[4]" :hidden="data.count[4]==0" :max="99">{{$t('default.after_sales')}}</el-badge></span>
                    </router-link>
                </li>
            </ul>
        </div>
        ...
        <div class="user_main">
            <div class="block_title">
                <span><router-link to="/user/order">{{$t('home.view_more')}}</router-link></span>
                {{$t('default.recent_orders')}}
            </div>
            <div class="x20"></div>
        ...
        <div class="user_main">
            <div class="block_title">
                <span><router-link to="/user/favorite">{{$t('home.view_more')}}</router-link></span>
                {{$t('default.recent_collection')}}
            </div>
            <div class="x20"></div>
