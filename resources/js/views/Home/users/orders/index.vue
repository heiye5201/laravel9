<template>
    <div class="user_order">
            ...
            <div class="block_title">
                <div class="user_order_search">
                    <ul>
                        ...
                        <li style="padding:0;">
                            <el-input v-model="data.params.order_no" :placeholder="$t('home.carts.order_sn')" />
                        </li>
                        <li style="padding:0;" >
                            <el-date-picker
                ...
                <em style="line-height:27px">{{$t('user_center.order.my_order')}}</em>
            </div>
            <div class="x20"></div>
              ...
              <div class="order_list" v-if="data.list.length>0">
                <div class="order_item" v-for="(v,k) in data.list" :key="k">
                    <div class="order_item_title">
                        ...
                        {{$t('home.carts.order_sn')}}：{{v.order_no||'-'}}
                    </div>
                    ...
                    <div class="order_item_btn" v-show="v.order_status!=6 || v.order_status !=0">
                        <div class="default_btn orderbtn" v-if="v.order_status==1" @click="edit_order_status(v.id)">{{$t('user_center.order.cancel')}}</div>
                        <div class="success_btn orderbtn" v-if="v.order_status==1" @click="pay_order(v.id)">{{$t('user_center.order.pay')}}</div>
                        <div class="default_btn orderbtn" v-if="v.order_status>2 && v.order_status != 5 && v.refund_status != 3 " @click="$router.push('/user/order/'+v.id)">{{$t('user_center.order.view_logistics')}}</div>
                        <div class="error_btn orderbtn" v-if="v.order_status==3" @click="edit_order_status(v.id,4)">{{$t('user_center.order.confirm')}}</div>
                        <div class="gray_btn orderbtn" v-if="v.order_status==4" @click="$router.push('/user/comment/add/'+v.id)">{{$t('user_center.order.comment')}}</div>
                        <div class="warn_btn orderbtn" v-if="v.order_status>=2 && v.order_status !=5 && v.refund_status!=2" @click="$router.push('/user/order/refund/'+v.id)">{{$t('user_center.order.after_sale')}}</div>
                        <div class="warn_btn orderbtn" v-if="v.order_status==5 || v.refund_status==2" @click="$router.push('/user/order/refund_form/'+v.id)">{{$t('user_center.order.view_after_sale')}}</div>
                    </div>
                </div>
        ...
        <!-- 物流 -->
        <a-modal v-model="visible" :title="$t('user_center.order.logistics')" :footer="null">
...
export default {
    ...
    setup(props) {
        ...
        const data = reactive({
            ...
            list:[],
            searchConfig:[
                {label:proxy.$t('user_center.order.all'),value:-1},
                {label:proxy.$t('user_center.order.cancel_order'),value:0},
                {label:proxy.$t('user_center.order.wait_pay'),value:1},
                {label:proxy.$t('user_center.order.wait_send'),value:2},
                {label:proxy.$t('user_center.order.confirm_send'),value:3},
                {label:proxy.$t('user_center.order.wait_comment'),value:4},
                {label:proxy.$t('user_center.order.after_sale_order'),value:5},
                {label:proxy.$t('user_center.order.completion_order'),value:6},
            ],
            order_info:{
                delivery_list:[],
            },
