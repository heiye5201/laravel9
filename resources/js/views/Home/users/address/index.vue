    ...
    <div class="user_address">
        <div class="user_main table_lists">
            <div class="block_title">
                <span><div class="btn" @click="addVis = true;isEdit=false">{{$t('user_center.address.add')}}</div></span>
                {{$t('user_center.address.receiving')}}
            </div>
            <div class="x20"></div>
            <div class="address_list" v-if="data.addresses.length>0">
                <ul>
                    <li v-for="(v,k) in data.addresses" :key="k">
                        ...
                        <div class="handle"><span v-if="!v.is_default" @click="set_default(v.id)">{{$t('user_center.address.default')}}</span><em v-if="!v.is_default">|</em><span @click="openEdit(v.id)">{{$t('user_center.address.edit')}}</span>|<span @click="del(v.id)">{{$t('user_center.address.remove')}}</span></div>
                    </li>
                </ul>
            </div>
...
export default {
    ...
    setup(props) {
        ...
        // 表单配置
        const addColumn = [
             {label:proxy.$t('user_center.address.addressee'),value:'receive_name'},
             {label:proxy.$t('user_center.address.phone'),value:'receive_tel'},
             {label:proxy.$t('user_center.address.company'),value:'area',type:'cascader',props:{emitPath:true,label:'name',value:'id'}},
             {label:proxy.$t('user_center.address.address_detail'),value:'address'},
             {label:proxy.$t('user_center.address.set_default'),value:'is_default',type:'switch'},
        ]
        const data = reactive({
            addresses:[],
            params:{
