<template>
    <div class="create_order_1 w1200">
        <div class="step_bar">
            <div class="step">
                <div class="item check"><el-icon><Shop /></el-icon>{{$t('home.carts.my_cart')}}</div>
                <div class="item"><el-icon><MessageBox /></el-icon>{{$t('home.carts.address')}}</div>
                <div class="item"><el-icon><Money /></el-icon>{{$t('home.carts.select_pay')}}</div>
        ...
        <div class="cart_th" v-if="data.params.total>0">
            ...
            <span class="attr">{{$t('home.carts.size')}}</span>
            <span class="price">{{$t('home.carts.price')}}</span>
            <span class="num">{{$t('home.carts.number')}}</span>
            <span class="total">{{$t('home.carts.total')}}</span>
            <span class="handle">{{$t('home.carts.handle')}}</span>
        </div>

        <div class="cart_table" v-if="data.params.total>0">
            <div class="store_list" v-for="(v,k) in data.list" :key="k">
                <div class="store_title"><el-checkbox :indeterminate="v.css" :model-value='v.checked' @change="onCheckAllStoreChange(k)">{{v.store_name}}</el-checkbox><span class="open_store" @click="$router.push('/store/'+v.store_id)">{{$t('home.carts.store')}}</span></div>
                <div class="goods_list">
                    <ul>
                        <li v-for="(vo,key) in v.cart_list" :key="key">
                            ...
                            <span class="handle" @click="del(vo.cart_id)">{{$t('home.carts.remove')}}</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="cart_th" v-if="data.params.total>0">
            ...
            <span class="total btcss" >{{$t('home.carts.selected')}} <font color="#ca151e">{{data.allCount}}</font> {{$t('home.carts.count')}}，{{$t('home.carts.total_count')}} <font color="#ca151e">{{R.formatFloat(data.allPrice)}} </font>{{$t('home.carts.price_unit')}}</span>
            <span class="handle" style="width:140px;text-align:right;"><div class="error_btn" style="padding:5px 30px;" @click="buy">{{$t('home.carts.buy_action')}}</div></span>
        </div>
