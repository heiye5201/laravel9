            ...
            <div class="block or">
                ...
                <div class="integrral_info" v-show="isLogin">
                    ...
                    <div class="myintegral">
                        ...
                        {{$t('user.my_point')}}：{{data.userInfo.integral||'0.00'}}
                    </div>
                </div>

                <div class="interal_person" v-show="isLogin">
                    <router-link to="">{{$t('home.integral.center')}}</router-link>
                </div>
            </div>
            <div class="block or2">
                <div class="integral_block_title">{{$t('home.personal_tailor')}}</div>
                <div class="integral_block_img"><img :src="require('@/assets/Home/integral_01.png').default" /></div>
                <div class="integral_block_desc">{{$t('home.personality_service')}}</div>
            </div>
            <div class="block or3">
                <div class="integral_block_title">{{$t('home.24h_service')}}</div>
                <div class="integral_block_img"><img :src="require('@/assets/Home/integral_02.png').default" /></div>
                <div class="integral_block_desc">{{$t('home.24h_answer_service')}}</div>
            </div>
            <div class="block or4">
                <div class="integral_block_title">{{$t('home.first_join')}}</div>
                <div class="integral_block_img"><img :src="require('@/assets/Home/integral_03.png').default" /></div>
                <div class="integral_block_desc">{{$t('home.join_limit')}}</div>
            </div>
        </div>
        ...
        <div class="integral_content">
            <div class="integral_goods_list w1200" v-if="data.recommend.length>0">
                <div class="integral_goods_list_title">
                    {{$t('home.hot')}}
                    <span>
                        <router-link to="/integral/search">{{$t('config.more')}} >></router-link>
                    </span>
                </div>
                        ...
                        <li v-for="(v,k) in data.recommend" :key="k">
                            <router-link :to="'/integral/goods/'+v.id">
                                <dl>
                                    ...
                                    <dd class="integral_goods_list_item_price"><el-icon><Coin /></el-icon> {{v.goods_price}}
                                        <em>{{$t('home.goods.market_value')}}:{{v.goods_market_price}}</em></dd>
                                    <dd class="integral_goods_list_item_btn">{{$t('home.goods.point_buy')}}</dd>
                                </dl>
                            </router-link>
                        </li>
                    </ul>
            ...
            <div class="integral_goods_list w1200" v-for="(v,k) in data.list" :key="k">
                <div class="integral_goods_list_title">
                    {{v.name}}<span><router-link :to="'/integral/search'">{{$t('config.more')}} >></router-link></span>
                </div>
                <div class="integral_goods_list_item">
                    <ul>
                        <li v-for="(vo,key) in v.integral_goods" :key="key">
                            <router-link :to="'/integral/goods/'+vo.id">
                                <dl>
                                    <dt><img :src="vo.goods_master_image" /></dt>
                                    <dd class="integral_goods_list_item_title" :title="vo.goods_name">{{vo.goods_name}}</dd>
                                    <dd class="integral_goods_list_item_price"><el-icon><Coin /></el-icon> {{vo.goods_price}}
                                        <em>{{$t('home.goods.market_value')}}:{{vo.goods_market_price}}</em></dd>
                                    <dd class="integral_goods_list_item_btn">{{$t('home.goods.point_buy')}}</dd>
                                </dl>
                            </router-link>
                        </li>
