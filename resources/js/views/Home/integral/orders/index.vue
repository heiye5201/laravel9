<template>
    <div class="create_order_1 w1200">
        ...
        <div class="block">
            ...
            <div class="sum_block">
                <div class="total">{{$t('home.carts.total_point')}}：<span>￥{{data.order.goods_price || (data.order.goods_price*$route.params.buy_num) || 0.00}}</span>( {{$t('freight.free')}} )</div>
                <div :class="loading?'btn hide':'btn'" @click="createOrder">{{data.loading?$t('home.loading'):$t('home.integral.pay')}}</div>
                <div class="clear"></div>
            </div>
        </div>
        <!-- 预生成订单信息 E -->
